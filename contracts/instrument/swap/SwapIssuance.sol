...
contract SwapIssuance is IssuanceBase {
    ...
    function initialize(address instrumentManagerAddress, address instrumentAddress, uint256 issuanceId,
        ...
        _issuanceProperty.issuanceDueTimestamp = now.add(_sip.duration);
        ...
        emit AssetTransferred(_issuanceProperty.issuanceId, ENGAGEMENT_ID, Transfer.TransferType.Inbound, makerAddress, makerAddress,
            _sip.inputTokenAddress, _sip.inputAmount, "Input in");
        transfersData = Transfers.encode(transfers);

        // Create payable 1: Custodian --> Maker
        _createPayable(1, ENGAGEMENT_ID, address(_issuanceEscrow), makerAddress, _sip.inputTokenAddress,
    ...
    function engage(address takerAddress, bytes memory /** takerData */)
        ...
        emit AssetTransferred(_issuanceProperty.issuanceId, ENGAGEMENT_ID, Transfer.TransferType.Inbound, takerAddress,
        ...
        emit AssetTransferred(_issuanceProperty.issuanceId, ENGAGEMENT_ID, Transfer.TransferType.Inbound, _issuanceProperty.makerAddress,
            takerAddress, _sip.inputTokenAddress, _sip.inputAmount, "Output out");
